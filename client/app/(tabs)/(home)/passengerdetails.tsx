...
const passengerdetails = () => {
  ...
  const handleFormChange = (count: number, data: any) => {
    const updatedFormData = [...formData];
    updatedFormData[count] = data;
  ...
  const returnPassengerFunc = (count: number, type: string) => {
    ...
    const result = Array.from({ length: count }).map((item, ind) => (
      <Passengerdetails
        index={ind}
        key={ind}
        type={type}
  ...
  const handleContinue = () => {
    ...
    let totalTravellers =
      travelInfo?.adults + travelInfo?.children + travelInfo?.infants;
    console.log('toto', totalTravelers);
    if (formData.length !== totalTravelers) {
      isValid = false;
      errorMessage = `Please fill all the forms`;
      alert(errorMessage);
      return;
    }
