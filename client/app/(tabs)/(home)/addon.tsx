...
const addon = () => {
  const { flightSearchInfo, flightBookingPrice } = useFlightAppData(
    (state) => state
  );
  ...
  const handleSkip = () => {
    if (activeNavItem == "Seat") {
      setNavItem("Meal");
    } else if (activeNavItem == "Meal") {
      setNavItem("Baggage");
    } else {
      ...
      <View style={styles.topnavbarMainContainer}>
        <View style={styles.arrowContainer}>
          ...
          <TouchableOpacity onPress={handleSkip}>
            <Text style={{ fontWeight: "500", color: "#01493E" }}>Skip</Text>
          </TouchableOpacity>
        </View>
